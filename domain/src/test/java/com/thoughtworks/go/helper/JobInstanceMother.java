...
public class JobInstanceMother {
    ...
    public static JobInstance buildEndingWithState(JobState endState, JobResult result, String jobConfig) {
        ...
        ZonedDateTime scheduledDate = ZonedDateTime.now().minusMinutes(5);
        instance.setScheduledDate(Date.from(scheduledDate.toInstant()));
        ...
        for (JobState stateToCompareTo : orderedStates) {
            if (endState.compareTo(stateToCompareTo) >= 0) {
                instance.changeState(stateToCompareTo, Date.from(scheduledDate.toInstant()));
                scheduledDate = scheduledDate.plusMinutes(1);
            }
        }
    ...
    private static void setTransitionConditionally(JobInstance instance, JobState state, ZonedDateTime date) {
        ...
        if (transition != null) {
            ...
            instance.changeState(state, Date.from(date.toInstant()));
        }
    }
